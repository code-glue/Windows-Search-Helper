...
set NullPersistentHandler={098f2470-bae0-11cd-b579-08002b30bfeb}
...
if defined CurrentPersistentHandler (
...
REM Display message and exit if there is already a null filter.
if /i "!CurrentPersistentHandler!" == "!NullPersistentHandler!" (
    echo.Windows Search null filter already registered for file extension: !Extension!
    set "ExitCode=0"
    goto ExitPause
)
...
call :SetErrorLevel 0
reg add "!RegKeyHKLM!" /ve /d "!NullPersistentHandler!" /f >nul
if %ErrorLevel% neq 0 echo>&2.Registry key: "!RegKeyHKLM!" & goto ExitPause
echo.Registered Windows Search null filter for file extension: !Extension!
set ExitCode=0
...
echo.
echo.  C:\^>%ThisFileNameNoExt% "tar"
echo.    Registers a Windows Search null filter for .tar files.
echo.
echo.  C:\^>%ThisFileNameNoExt% .tar
echo.    Registers a Windows Search null filter for .tar files.
...
echo.
echo.Description:
echo.  Configures Windows Search to ignore the contents of all files with the
echo.  specified extension (helpful for binary files). If the file extension
